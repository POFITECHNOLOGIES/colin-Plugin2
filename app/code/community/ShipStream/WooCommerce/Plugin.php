...
class ShipStream_WooCommerce_Plugin extends Plugin_Abstract
{
    ...
    public function importOrderEvent(Varien_Object $data)
    {
        ...
        try {
            if ($script = $this->getConfig('filter_script')) {
                ...
                try {
                    $newOrderData = $this->applyScriptForOrder($script, $newOrderData, ['wooCommerceOrder' => $wooCommerceOrder], $output);
                } catch (Exception $e) {
    ...
    public function shipmentPackedEvent(Varien_Object $data)
    {
        ...
        if (!in_array($clientOrder['status'], ['wc-submitted', 'wc-failed-to-submit'])) {
        ...
        if ($statuses) {
            do {
                ...
                foreach ($data as $orderData) {
                    ...
                    $this->addEvent('importOrderEvent', ['increment_id' => $orderData['id']]);
